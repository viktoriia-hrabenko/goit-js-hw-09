...
btnCreatePromise.addEventListener("click", (e)=>{
    ...
    for(let i = 0; i < amount.value; i++)createPromise(1 + i, firstDelay + i * delayStep).then(({ position , delay: delay2  })=>{
        (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${delay2}ms`);
    }).catch(({ position , delay: delay3  })=>{
        (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${delay3}ms`);
    });
});
